...
export class Blocker extends DisplayElement {
  ...
  block(reason) {
    if (this.children[reason]) {
      this.show(this.children[reason]);
    }
  }
  ...
  /**
   * Create a new Blocker screen with an error message and `show()` it.
   * @param error An error that has been caught.
   * @param devError An alternative error message to display in dev mode to help resolve the problem.
   */
