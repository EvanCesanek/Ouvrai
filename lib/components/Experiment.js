...
export class Experiment {
  ...
  createTrialSequence(blocks, append = false) {
    ...
    for (let [blockNumber, bk] of blocks.entries()) {
      ...
      if (!validBlock) {
        console.error(
          'ERROR: createTrialSequence() requires all trial variable arrays have the same length'
        );
        return -1;
      }
      if (numTrials === undefined) {
        numTrials = 1;
        console.warn(
          'WARNING: createTrialSequence() received no arrays of trial variables. Creating one empty trial per block.'
        );
      }
