...
export class Experiment {
   ...
   * you may want to increase the second argument to give the emulators more time to start.
   *
   * @param {string} nextState State to transition into (default 'WELCOME' for VR, 'SETUP' for non-VR). Set null to use defaults.
   * @param {integer} retries If in dev mode, number of times to retry if emulators are slow to start (3 seconds/try).
   */
  processSignIn(nextState = null, retries = 3) {
    this.state.once(() => {
      this.firebase
        .signOut()
        .then((previousUser) => {
        ...
        .catch((err) => {
          ...
          if (import.meta.env.DEV && err.code === 'auth/internal-error') {
            ...
            Seeing this message often? Try setting the 'retries' argument of <b>processSignIn()</b> greater than 3.</p>`;
          }
