...
import { AudioListener, PMREMGenerator, TextureLoader } from 'three';
...
export class Experiment {
  constructor({
    ...
    this.cfg.experiment = import.meta.env.VITE_EXPERIMENT_NAME;
    this.cfg.demo = demo;
    this.trials = trials;
    this.trialNumber = startTrialNumber;
    this.cfg.cssBackground = document.body.style.background = cssBackground;
    ...
    this.consent = new Consent({ vr: this.cfg.requireVR });
  ...
  processSignIn(nextState) {
    if (this.firebase.uid) {
      ...
      if (this.cfg.requireVR) {
        ...
        this.state.next(nextState || this.state.WELCOME);
      } else {
        DisplayElement.show(document.getElementById('panel-container'));
        this.state.next(nextState || this.state.SETUP);
      }
    }
  }
  ...
  createTrialSequence(blocks, append = false) {
    ...
    for (let [blockNumber, bk] of blocks.entries()) {
      ...
      let numTrials = Object.values(bk)[0]?.length;
      const validBlock = Object.values(bk).every(
        (element) => element.length === numTrials
      );
      if (!validBlock) {
        throw new Error(
          'Error: All trial-variable arrays must have the same length.'
        );
      }
      if (numTrials === undefined) {
        numTrials = 1;
        console.warn(
          'Warning: No trial-variable arrays supplied. Creating one empty trial per block.'
        );
      }
  ...
  async initVR({ handTracking = false, controllerModels = false }) {
    ...
    this.sceneManager.renderer.xr.addEventListener(
      ...
      function () {
        ...
        this.xrReferenceSpace.addEventListener &&
    ...
    // we need these extra listeners because of context problem...
    // e can't bind(this) on the connect handlers because we need the default context (grip/ray1, grip/ray2)
  ...
  handleRightGripConnected(e) {
    ...
    this.cfg.supportHaptic =
      Object.prototype.hasOwnProperty.call(gamepad, 'hapticActuators') &&
  ...
  handleRightRayConnected(e) {
    ...
    if (this.UI?.xrPointer?.isObject3D) {
      this.ray.add(this.UI.xrPointer);
    }
