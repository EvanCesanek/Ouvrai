...
export class Experiment {
  ...
  replay;
  /**
   * The {@link https://developer.mozilla.org/en-US/docs/Web/API/XRInputSource/gripSpace grip space}
   * is a three.js Group returned by
   * {@link https://threejs.org/docs/#api/en/renderers/webxr/WebXRManager.getControllerGrip WebXRManager.getControllerGrip()}.
   * @type {Group}
   * @public
   */
  grip;
  ...
  async #initVR() {
    ...
    this.ray1.addEventListener('pinchstart', () => {
      console.log('pinchstart received on ray1');
      this.ray1.userData.isSelecting = true;
    });
    this.ray1.addEventListener('pinchend', () => {
      console.log('pinchend received on ray1');
      this.ray1.userData.isSelecting = false;
    });
    this.ray2.addEventListener('pinchstart', () => {
      console.log('pinchstart received on ray2');
      this.ray2.userData.isSelecting = true;
    });
    this.ray2.addEventListener('pinchend', () => {
      console.log('pinchend received on ray2');
      this.ray2.userData.isSelecting = false;
    });
    ...
    if (this.cfg.handTracking) {
      ...
      // );
      // this.hand1.add(new module.OculusHandPointerModel(this.hand1, this.ray1));
      // this.hand2.add(new module.OculusHandPointerModel(this.hand2, this.ray2));
  ...
  onConnectedController(e, i) {
    ...
    // console.log('----');
    let grip, ray, hand;
    this.cfg.xrInputProfiles = e.data.profiles;
    let hapticActuators = e.data.gamepad?.hapticActuators;
   ...
   * This was required because a hand can lose tracking and it will be disconnected,
   * But strangely (on Meta Quest 2 at least) a "dummy hand" input source will be connected in its place.
   * Unfortunately three.js does not know the difference and the rendered dummy hand will jump to the local-floor origin.
   * There is no easy way to test if tracking is live without the XRFrame method getPose().
   * Note the current XRFrame is only available in the requestAnimationFrame callback, so we call this function there,
   * instead of doing it in the onControllerConnected/Disconnected methods where you'd think we should.
   *
   * Another possible workaround: In testing on the Meta Quest 2 in April 2023, the inputSource.gamepad property was null on a real hand, but not on the dummy hand. This is unusual though so I didn't rely on it.
  ...
  /**
   * Check if we are in VR and if each hand is connected (i.e. visible)
  ...
  controllerInterrupt(left = true, right = true) {
    return (
      this.sceneManager.renderer.xr.isPresenting &&
      ((left && (!this.leftGrip || !this.leftGrip.visible)) ||
        (right && (!this.rightGrip || !this.rightGrip.visible)))
    );
  }
