...
export class Experiment {
  ...
  sceneManager;
  /**
   * Create a survey in your experiment.
   * @example
   * // File: experiments/<studyname>/src/index.js
  ...
  survey;
  /**
   * Create a brief instructions panel overlaid on the top-left of the scene.
   * @example
   * // File: experiments/<studyname>/src/index.js
  ...
  instructions;
  /**
   * (DEV) Initialize a Replay machine by setting `devOptions.replay: true` in the configuration object.
   * This allows you to load JSON files into the experiment and replay trajectories from the data using the three.js animation system.
   *
   * Important: You must set up `handleReplayInfo()` and `handleReplayTrial()` functions in the main experiment code. See `vr-gen` template for example usage.
   * @type {Replay}
   */
  replay;
  /**
   * (VR) Ouvrai keeps track of which WebXR input source {@link https://developer.mozilla.org/en-US/docs/Web/API/XRInputSource/gripSpace grip space}
  ...
  leftGrip;
  /**
   * (VR) Ouvrai keeps track of which WebXR input source {@link https://developer.mozilla.org/en-US/docs/Web/API/XRInputSource/targetRaySpace ray space}
  ...
  leftRay;
  /**
   * (VR) Ouvrai keeps track of which WebXR input source {@link https://developer.mozilla.org/en-US/docs/Web/API/XRInputSource/hand hand object}
  ...
  /**
   * You must create a new Experiment at the start of any Ouvrai experiment code.
   * Use the `cfg` argument to specify the desired configuration options.
   * @example
   * // File: experiments/<studyname>/src/index.js
   * function main(){
   ...
   *   });
   * }
   * @param {Object} cfg Experiment configuration object; becomes {@link Experiment#cfg cfg} property after initialization. Saved to Firebase at consent and completion.
   ...
   * @param {String} [cfg.backgroundColor] Background color of experiment, default 'dimgray'
   * @param {Number} [cfg.fixedAspect] Specify a fixed aspect ratio for the viewport
  ...
  constructor(cfg) {
    ...
    /**
     * (VR) This array is used to keep track of all current XR input sources.
     * @type {XRInputSource[]}
     * @private
     */
    this.inputSources = [];
   ...
   * @param {Number} bonusPayment If you want to notify the participant of a bonus payment, specify it here (assumes USD).
   */
  complete(bonusPayment = 0.0) {
    this.goodbye.updateGoodbye(
      this.firebase.uid,
      this.cfg.platform,
      bonusPayment
    );
  ...
  waitForAuthentication(retries = 3) {
    ...
    this.state.once(() => {
      ...
      this.firebase
        .signOut()
        .then((previousUser) => {
          // Save the previously logged-in user info (if there was one)
          if (previousUser) {
            console.log(
        ...
        .then(() => {
          // Hide consent form and show the three.js scene
        ...
        .catch((err) => {
          ...
          // Catch slow-to-start Firebase emulators (typically when ouvrai dev <studyname> is first called)
