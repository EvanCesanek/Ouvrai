...
export class Firebase {
  ...
  async signInAnonymously(retries = 1) {
    if (this.demo) {
      this.uid = 'demo';
      return;
    }

    // Retrying with delay via promise chains
    // Because Auth emulator takes a moment longer than Database
    ...
    if (this.auth.currentUser?.uid) {
      ...
      for (let i = 0; i < retries; i++) {
        p = p
          .catch(() => {
            console.log(`Signing out ${this.auth.currentUser?.uid}`);
            signOut(this.auth);
          })
          .catch(rejectDelay);
      }
  ...
  async recordCompletion() {
    ...
    try {
      await set(
        ref(
          this.database,
          `/workers/${this.workerId}/${this.experiment}/${this.uid}`
        ),
        {
          completed: true,
        }
