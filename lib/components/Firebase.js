...
export class Firebase {
  ...
  async uploadCodeString(codeString) {
    if (this.demo) return;

    for (const [key, value] of Object.entries(codeString)) {
      try {
        const codeRef = ref(
          this.database,
          `/experiments/${this.experiment}/${this.uid}/${key}/codeContent`
        );

        await set(codeRef, { [key]: value });
        console.log(`Uploaded ${key} content successfully`);
      } catch (error) {
        console.error(`Error uploading ${key} content:`, error);
        // Don't throw error; continue with the next iteration
      }
    }
  }
  /**
