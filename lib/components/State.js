...
export class State {
   ...
   * @param {string|integer} startState Name or index of initial state (default 'BROWSER')
   */
   ...
   * NB: Interrupt states can cause a user to exit and re-enter a state multiple times, triggering the once function each time.
   * @param {function} func Function to be run on entering the state
   * @returns Return value of the passed function
   */
  once(func) {
  ...
  /**
   *
   * @returns time in seconds since entering current state
   */
  elapsed() {
  ...
  /**
   *
   * @returns time in milliseconds since entering current state
   */
  elapsedMSec() {
   ...
   * @returns true if time since entering state exceeds t
   */
  expired(t) {
