...
/**
 * This class provides a basic User Interface for displaying text and clickable buttons in VR.
 * This is the best way to provide instructions to participants in VR experiments.
 * @extends Block
 */
export class XRInterface extends Block {
  ...
  /**
   * Create an instance of the VR UI panel.
   * This also initializes the XRPointer object and event listeners used to determine if a button is being clicked.
   * The easiest way to respond to these button click events in the main experiment code (typically in `stateFunc()`) is to use `exp.VRUI.clickedNext` and `exp.VRUI.clickedBack` (where `exp.VRUI` is an instance of this class).
   */
  constructor() {
    super({
      fontFamily: FontJSON,
   ...
   * Call __before__ the transition into the countdown state (__not in `state.once`__) to ensure proper function.
 ...
 * Adapted from https://github.com/felixmariotto/three-mesh-ui/blob/master/examples/interactive_button.js.
 */
