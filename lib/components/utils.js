...
/**
 * Cartesian product of arrays. Helpful for creating factorial designs to feed into `new Block()`.
...
/**
 * Round an angle to the nearest step of increment `2*pi/n`.
 * @param {Number} theta Angle in radians (use atan or atan2 if you have XY coords)
 * @param {Number} n Number of discrete angular steps from 0 to 2*pi, default 4
...
/**
 * Convert a named color to the equivalent hex string.
 * @param {String} str Color name as a string (e.g., 'aliceblue')
 ...
 * @returns {Number[]} Two-element array [targetAngle, distance]
 */
...
/**
 * Class that provides some simple formatting of DateTime strings
 */
export class DateTime {
  /**
   * Format a DateTime string as: yyyy-mm-dd-hh-nn-ss
   * @param {String} join Character(s) used to join the separate pieces of the DateTime string.
  ...
  /**
   * Absolute measure of current time.
   * @returns {Number} Milliseconds elapsed since midnight, January 1, 1970 Universal Coordinated Time (UTC)
   */
  static absolute() {
    return Date.now();
  }
  /**
   * Relative measure of current time.
   * @returns {Number} Milliseconds elapsed since page load.
   */
  static relative() {
...
/**
 * Sample from a truncated normal distribution. Provides randomness without possibility of absurd values.
 * @param {Number} min Minimum value
 * @param {Number} max Maximum value
 * @param {Number} truncateStd Number of standard deviations beyond which to truncate (resample)
 * @returns {Number} Randomly sampled value
 */
...
/**
 * Produce an array of equally spaced values, spanning from `startValue` to `stopValue` (inclusive)
 * @param {Number} startValue First value in the return array
 * @param {Number} stopValue Last value in the array
 * @param {Number} cardinality Number of values to return, default 2 (must be greater than 1)
 * @returns {Number[]} Array of equally spaced values
 */
export function linspace(startValue, stopValue, cardinality = 2) {
  if (cardinality < 2) {
    throw new Error('linspace cardinality must be greater than 1');
  }
  let arr = [];
 ...
 * @param {Number} p.reps Repetitions (default 1)
...
/**
 * Get the XRHand kinematic chain.
 * @returns {Object} Hierarchical representation of the XRHand kinematic chain.
 */
export function xrHandKinematicChain() {
  return {
    wrist: {
      'thumb-metacarpal': {
