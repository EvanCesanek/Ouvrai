 ...
 * @returns Random `N`-digit numeric string, left-padded with zeros
 */
 ...
 * @param {number|false} p.distanceThresh Maximum allowable distance
 * @param {number|false} p.angleThresh Maximum allowable angle difference (around `axis`)
 * @param {string} p.axis World axis around which to check angle alignment ('x', 'y', or 'z')
 * @returns true if o1 and o2 are within the specified thresholds, false otherwise
 */
...
/**
 * Toggles object visibility within no-feedback zone centered on home and draws expanding ring.
 * Warning: Assumes XZ distance from home.
 * @param {Object3D} tool
 * @param {Object3D} home
 * @param {Object3D} zone
 * @param {boolean} forcevisible
 */
export function feedbackShowHide(tool, home, zone, forcevisible = false) {
  zone.visible = true;
