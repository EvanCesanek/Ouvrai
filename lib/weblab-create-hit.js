...
var configPath = join(__dirname, '../experiments', expName, 'wjs_config.js');
...
async function overwriteMTurkLayout() {
  ...
  replaceOptions = {
    ...
    to: [
      `        var expName = '${expName}'; ${timestampComment}`,
      `        var taskURL = 'https://${siteName}.web.app'; ${timestampComment}`,
      `        var databaseURL = '${config.databaseURL}'; ${timestampComment}`,
    ],
  };
...
function generateHIT(html_q, quals) {
  ...
  if (!(config.sandbox || options.sandbox)) {
    // typically we don't want batch labels in Sandbox mode
